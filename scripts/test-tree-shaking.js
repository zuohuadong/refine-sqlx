...
const tempDir = join(tmpdir(), `tree-shaking-test-${randomBytes(8).toString('hex')}`);
...
for (const pkg of packages) {
  ...
  if (existsSync(packageJsonPath)) {
    ...
    if (packageJson.sideEffects === false) {
      ...
      console.log(`  ⚠️  sideEffects array specified: ${packageJson.sideEffects.join(', ')}`);
    } else {
      console.log('  ❌ sideEffects not set to false - Tree-shaking may not work optimally');
    }

    // Check exports field for proper ESM/CJS dual package
    if (packageJson.exports) {
      console.log('  ✅ Package exports defined - Supports conditional exports');
      ...
      console.log('  ⚠️  No package exports - May not support conditional imports');
    }
    ...
    if (packageJson.module) {
      ...
      console.log('  ⚠️  No module field - Bundlers may not detect ESM version');
    }
  }
...
`;
  ...
  try {
    writeFileSync(testNamespaceImport, `
