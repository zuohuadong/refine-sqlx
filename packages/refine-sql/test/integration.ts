  ...
  return () => {
    ...
    describe(`${clientName} Data Provider Integration Tests`, () => {
      ...
      it('should handle getList with pagination and filtering', async () => {
        ...
        expect(filteredResult.data.every((user: any) => user.age >= 30)).toBe(
          true
        );
      });
      ...
      it('should handle update operations', async () => {
        ...
        const updateResult = await dataProvider.update({
          ...
          variables: { name: 'Updated User', age: 26 },
        });
      ...
      it('should handle delete operations', async () => {
        ...
        const listResult = await dataProvider.getList({ resource: 'users' });
      ...
      it('should handle complex filtering and sorting', async () => {
        ...
        for (const user of users) {
          await dataProvider.create({ resource: 'users', variables: user });
        }
        ...
        expect(ageRangeResult.data.every((user: any) => user.age >= 30)).toBe(
          true
        );
      });
    });
    ...
    describe(`${clientName} Error Handling`, () => {
      ...
      it('should handle invalid SQL queries', async () => {
        await expect(
          client.query({ sql: 'SELECT * FROM non_existent_table', args: [] })
        ).rejects.toThrow();
      });

      it('should handle malformed SQL in data provider', async () => {
        await expect(
          dataProvider.getList({ resource: 'invalid_table_name' })
        ).rejects.toThrow();
      });
    });
  };
