...
export default function (db: string, options?: import('./types/config').SQLiteOptions): SqlClientFactory;
...
export default function (
  ...
  const connect = withErrorHandling(
    async (): Promise<SqlClient> => {
      ...
      if (supportedRuntime === 'cloudflare-worker') {
        ...
        try {
          ...
          const instance = new DatabaseSync(db, options?.node as NodeDatabaseOptions);
      ...
      try {
        ...
        const instance = new (Database as any).default(db, options?.['better-sqlite3']);
        const createBetterSQLite3Adapter = (
          await import('./adapters/better-sqlite3')
        ).default;
