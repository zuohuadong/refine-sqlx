...
async function newUsageExamples() {
  ...
  // Relationship queries (same API as @refine-sqlx/sqlx)
  const postsWithAuthors = await dataProvider
    .from('posts')
    .withBelongsTo('author', 'users', 'userId')
    .where('published', 'eq', true)
...
async function advancedExamples() {
  // Enhanced aggregation (new in @refine-sqlx/sql)
  ...
  // Batch processing (new in @refine-sqlx/sql)
  const allUsers = [];
...
async function performanceComparison() {
  ...
  console.log(`Bundle size: ~23kB (vs ~150kB with @refine-sqlx/sqlx)`);
