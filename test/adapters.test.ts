...
describe('Cloudflare D1 Adapter', () => {
  ...
  it('should execute queries correctly', async () => {
    ...
    const client = createCloudflareD1Adapter(mockD1 as any);
    const query: SqlQuery = { sql: 'INSERT INTO users (name) VALUES (?)', args: ['John'] };
  ...
  it('should handle batch operations', async () => {
    ...
    mockD1.batch.mockResolvedValue([
      ...
      { success: true, meta: { columns: ['id', 'name'], changes: 0 }, results: [[1, 'John']] }
    ]);
    ...
    expect(results[1]).toEqual({ columnNames: ['id', 'name'], rows: [[1, 'John']] });
  });
  ...
  it('should handle batch failures', async () => {
    ...
    await expect(client.batch!(queries)).rejects.toThrow('Batch query failed: Syntax error');
  });
});
...
describe('Bun SQLite Adapter', () => {
  ...
  it('should handle transactions', async () => {
    ...
    const result = await client.transaction!(async (tx) => {
      await tx.execute({ sql: 'INSERT INTO users (name) VALUES (?)', args: ['John'] });
      return 'success';
    });
  ...
  it('should rollback on transaction failure', async () => {
    const mockStmt = {
      run: vi.fn()
        .mockReturnValueOnce({ changes: 0 }) // BEGIN
        .mockImplementationOnce(() => { throw new Error('DB Error'); }) // INSERT fails
